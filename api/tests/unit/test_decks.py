...
def test_create_deck_returns_200_in_boundary_deck_limit(
        client, mock_db, db_refresh_sets_id
):
    test_cases = [
        (0, True, "first_deck"),        
        (10, True, "eleventh_deck"),      
        (15, True, "sixteenth_deck"),    
    ]
    for current_count, should_succeed, deck_name in test_cases:
        ...
        if should_succeed:
            assert response.status_code == 200, \
                f"Should succeed when user has {current_count} decks"
            mock_db.add.assert_called_once()
            mock_db.commit.assert_called_once()
            if current_count == 19:
                mock_db.refresh.assert_called_once()
            ...
            assert response.status_code == 400, \
                f"Should fail when user has {current_count} decks"
            assert "лимит" in response.json()["detail"].lower()
            mock_db.add.assert_not_called()
            mock_db.commit.assert_not_called()

def test_create_deck_returns_200_when_user_has_19_decks_creating_20th(
        client, mock_db, db_refresh_sets_id
):
    """Пользователь с 19 колодами должен иметь возможность создать 20-ю колоду (граница)."""
    # Arrange
    mock_user = make_user()
    ...
    filter_obj.first.return_value = mock_user
    filter_obj.count.return_value = 19 
